...
namespace MGroup.XFEM.IsoXFEM.ElementStructuralStiffnessComputations
{
    public class ElementStructuralStiffnessXFEMComputation : IElementStructuralStiffnessComputation
    {
		...
		private IElementSubcell[] subcells;
		public ElementStructuralStiffnessXFEMComputation(/*Matrix coordinatesOfTriangles,int[,] connectionOfTriangles*/ IElementSubcell[] subcells)
        {
        ...
        public Matrix ElementStructuralStiffnessComputation(Matrix coordinatesOfElement, Matrix elasticityMatrix, double thickness)
        {
            {               
				...
				#region this code is on SolidOnlyTriangulator2D.CreateSubTrianglesOfElement(...)
    ...
    //            //Create a list with Matrices which contain the coordinates of sub-triangles
    ...
    //            }
				#endregion
				//calculate gauss points of the triangles in natural coordinates
				List<Matrix> GP = new List<Matrix>();
                Matrix gp1 = Matrix.CreateZero(2, /*connectionOfTriangles.GetLength(0)*/ subcells.Length);
                Matrix gp2 = Matrix.CreateZero(2, /*connectionOfTriangles.GetLength(0)*/subcells.Length);
                Matrix gp3 = Matrix.CreateZero(2, /*connectionOfTriangles.GetLength(0)*/subcells.Length);
                for (int aa = 0; aa < 2; aa++)
                {
                    for (int bb = 0; bb < /*connectionOfTriangles.GetLength(0)*/ subcells.Length; bb++)
                    {
						...
						IList <double[]> tcoor= subcells[bb].VerticesNatural;
						gp1[aa, bb] = (tcoor[0][aa] + tcoor[1][aa]) / 2;
						gp2[aa, bb] = (tcoor[1][aa] + tcoor[2][aa]) / 2;
						gp3[aa, bb] = (tcoor[2][aa] + tcoor[0][aa]) / 2;
					}
                ...
                GP.Add(gp2);
                GP.Add(gp3);
				//Calculate stiffness Matrix using gauss points of the sub-triangles
				Matrix stiffness = Matrix.CreateZero(8, 8);
				for (int aa = 0; aa < /*tarea.Length*/subcells.Length; aa++)
                {
                    for (int bb = 0; bb < 3; bb++)
                    {
						...
						var areaSubcellNatural = Utilities.CalcPolygonArea(subcells[aa].VerticesNatural);
                        stiffness = stiffness + thickness * TransposeBL0 * elasticityMatrix * BL0 * (detJ / 3) * areaSubcellNatural /*tarea[aa]*/;
                    }
                }
                return stiffness;
            }
