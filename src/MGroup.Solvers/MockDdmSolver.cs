...
namespace MGroup.Solvers
{
	public class MockDdmSolver : IDdmSolver
	{
		...
		public void DistributeNodalLoads(IEnumerable<INodalLoad> subdomainLoads, Vector nodalLoadsVector, 
...
;			DofTable freeDofs = subdomainDofs.FreeDofs;
			foreach (INodalLoad load in subdomainLoads)
			{
				int freeDofIdx = freeDofs[load.Node, load.DOF];
				nodalLoadsVector[freeDofIdx] /= load.Node.SubdomainsDictionary.Count;
			}
		}
		...
		public void DistributeAllNodalLoads(Vector nodalLoadsVector, ISubdomainFreeDofOrdering subdomainDofs)
		{
			...
			{
				nodalLoadsVector[freeDofIdx] /= node.SubdomainsDictionary.Count;
			}
		}
