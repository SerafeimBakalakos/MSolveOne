...
{
    public class CrackBodyNormals2DObserver : ICrackObserver
    {
        ...
        public CrackBodyNormals2DObserver(CrackCurve2D crack, string outputDirectory, bool plotPseudoNormals)
        {
            this.crack = crack;
        ...
        public void Update()
        {
            ...
            {
                // Normals of vertices
                string pathVertexNormals = $"{outputDirectory}\\crack_curve_normals_vertices_{crack.ID}_t{iteration}.vtk";
                using (var writer = new VtkPointWriter(pathVertexNormals))
                {
                    var normalVectors = new Dictionary<double[], double[]>();
                    foreach (Vertex2D vertex in crack.Vertices)
                    {
            ...
            // Normals of cells
            string pathCellNormals = $"{outputDirectory}\\crack_curve_normals_cells_{crack.ID}_t{iteration}.vtk";
            using (var writer = new VtkPointWriter(pathCellNormals))
            {
                var normalVectors = new Dictionary<double[], double[]>();
                foreach (LineCell2D cell in crack.Cells)
                {
