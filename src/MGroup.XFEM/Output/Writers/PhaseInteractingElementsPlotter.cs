...
namespace MGroup.XFEM.Output.Writers
{
	public class PhaseInteractingElementsPlotter : IModelObserver
	{
		...
		private class OutputMesh : IOutputMesh
		{
			...
			private readonly SortedDictionary<int, VtkPoint> vertices  = new SortedDictionary<int, VtkPoint>();
			...
			public void AddElement(IXFiniteElement element, int intersectingPhaseID)
			{
				...
				{
					int newID = this.vertices.Count;
					verticesOfCell[n] = new VtkPoint(newID, element.Nodes[n].Coordinates);
					this.vertices[newID] = verticesOfCell[n];
