...
{
	public class StructuralFieldWriter_v2 : IResultsWriter
	{
		...
		public StructuralFieldWriter_v2(IXModel model, IXGeometryDescription discontinuity, string outputDirectory, 
			bool plotFields = true, bool plotTensorsAtGaussPoints = true, bool plotSmoothFields = true)
		{
			this.iteration = 0;
			this.model = model;
		...
		{
			var outputMesh = new ConformingContinuousMesh(model, discontinuity);
			var strainStressField = new SmoothStrainStressField_v2(model, algebraicModel, outputMesh);
			(Dictionary<int, double[]> strains, Dictionary<int, double[]> stresses)
				= strainStressField.CalcTensorsAtVertices(solution);
