...
namespace MGroup.MSolve.Meshes.Structured
{
    ...
    /// by dividing each Quad9 cell of an equivalent cartesian mesh into 4 subtriangles. The element indices are thus int[3], 
    /// where the first 2 entries are the index of the enclosing Quad9 of an equivalent cartesian mesh and the third entry is 
    /// 0-3 to denote one of the 4 subtriangles of said Quad9.
    /// </summary>
    public class UniformSimplicialSymmetricMesh2D : IStructuredMesh
    {
        private const int dim = 2;
        private const int numNodesPerElement = 3;
        private const int numSimplicesPerCartesianCell = 4;
        ...
        /// <summary>
        /// Given the index (i,j) of the first node of a Quad5 with coordinates=(-1,-1), this list contains, for each subtriangle 
        ...
        private UniformSimplicialSymmetricMesh2D(double[] minCoordinates, double[] maxCoordinates, int[] numNodes, int axisMajor,
            int axisMinor, int firstNodeID, int firstElementID)
        {
            ...
            triangle0NodeIdxOffsets[0] = new int[] { 1, 1 };
            triangle0NodeIdxOffsets[1] = new int[] { 0, 0 };
            triangle0NodeIdxOffsets[2] = new int[] { 2, 0 };
            elementNodeIdxOffsets.Add(triangle0NodeIdxOffsets);

			// The offsets of the nodes of the second triangle are
			var triangle1NodeIdxOffsets = new int[numNodesPerElement][];
            triangle1NodeIdxOffsets[0] = new int[] { 1, 1 };
            triangle1NodeIdxOffsets[1] = new int[] { 2, 0 };
            triangle1NodeIdxOffsets[2] = new int[] { 2, 2 };
            elementNodeIdxOffsets.Add(triangle1NodeIdxOffsets);

			// The offsets of the nodes of the third triangle are
			var triangle2NodeIdxOffsets = new int[numNodesPerElement][];
			triangle2NodeIdxOffsets[0] = new int[] { 1, 1 };
			triangle2NodeIdxOffsets[1] = new int[] { 2, 2 };
			triangle2NodeIdxOffsets[2] = new int[] { 0, 2 };
			elementNodeIdxOffsets.Add(triangle2NodeIdxOffsets);

			// The offsets of the nodes of the fourth triangle are
			var triangle3NodeIdxOffsets = new int[numNodesPerElement][];
			triangle3NodeIdxOffsets[0] = new int[] { 1, 1 };
			triangle3NodeIdxOffsets[1] = new int[] { 0, 2 };
			triangle3NodeIdxOffsets[2] = new int[] { 0, 0 };
			elementNodeIdxOffsets.Add(triangle3NodeIdxOffsets);
		}
        ...
        public class Builder
        {
            ...
            public UniformSimplicialSymmetricMesh2D BuildMesh()
            {
                Validate();
                (int majorAxis, int minorAxis) = ChooseAxes();
                return new UniformSimplicialSymmetricMesh2D(coordsMin.Copy(), coordsMax.Copy(), numNodes.Copy(),
