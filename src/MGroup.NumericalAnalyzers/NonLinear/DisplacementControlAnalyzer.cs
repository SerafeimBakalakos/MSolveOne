	...
	public class DisplacementControlAnalyzer : NonLinearAnalyzerBase
	{
		...
		public override void Solve()
		{
			...
			for (int increment = 0; increment < numIncrements; increment++)
			{
				...
				UpdateRhs(increment);
				ScaleModelConstraints(increment);
		...
		protected override void InitializeInternalVectors()
		{
			...
			//modelUpdater.ScaleConstraints(1.0 / (double)numIncrements); // This should be called here, but it actually breaks the test.
		}
		...
		private void ScaleModelConstraints(int currentIncrement)
		{
			...
			double scalingFactor = 1;
			modelUpdater.ScaleConstraints(scalingFactor);
		}
