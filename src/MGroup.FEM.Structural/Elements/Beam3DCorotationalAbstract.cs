...
namespace MGroup.FEM.Structural.Elements
{
	public abstract class Beam3DCorotationalAbstract : IFiniteElement, IEmbeddedElement
	{
		...
		protected Beam3DCorotationalAbstract(IList<Node> nodes, double youngModulus, double poissonRatio, double density, BeamSection3D beamSection)
		{
			this.nodes = nodes;
		...
		public abstract void UpdateState(double[] nodeDisplacements);
		...
		{
			UpdateState(dofEnumerator.GetTransformedDisplacementsVector(localDisplacements));
