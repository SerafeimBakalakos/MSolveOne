...
namespace MGroup.FEM.Structural.Elements
{
	...
	public class Shell8NonLinear : IStructuralFiniteElement
	{
		...
		public Shell8NonLinear(IShellMaterial material, IQuadrature3D quadratureForStiffness)
		{
			...
			{
				lastStresses[i] = new double[123];
				materialsAtGaussPoints[i] = (IShellMaterial)material.Clone();
			}
		}
		...
		private double[] UpdateForces(IElement element)
		{
			...
			for (int j = 0; j < nGaussPoints; j++)
			{
				for (int k = 0; k < 40; k++)
				{
					Fxk[j][k] = 0;
					for (int m = 0; m < 6; m++)
					{ Fxk[j][k] += BL[j][m, k] * lastStresses[j][m]; }
				}
			}
			for (int k = 0; k < 40; k++)
			{
		...
		private Matrix UpdateKmatrices(IElement element)
		{
			...
			for (int j = 0; j < nGaussPoints; j++)
			{
				...
				for (int k = 0; k < 3; k++)
				{
					for (int l = 0; l < 3; l++)
					{
						SPK_circumflex[3 * k + l, 3 * k + l] = lastStresses[j][l];
					}
					SPK_circumflex[3 * k, 3 * k + 1] = lastStresses[j][3];
					SPK_circumflex[3 * k, 3 * k + 2] = lastStresses[j][5];
					SPK_circumflex[3 * k + 1, 3 * k + 2] = lastStresses[j][4];

					SPK_circumflex[3 * k + 1, 3 * k] = lastStresses[j][3];
					SPK_circumflex[3 * k + 2, 3 * k] = lastStresses[j][5];
					SPK_circumflex[3 * k + 2, 3 * k + 1] = lastStresses[j][4];
				}
				...
				for (int k = 0; k < 9; k++)
				{
					for (int m = 0; m < 6; m++)
					{
						BL01plus1_2tSPKvec[j][k] += BL01plus1_2[j][m, k] * lastStresses[j][m];
					}
				}
		...
		public Tuple<double[], double[]> CalculateStresses(IElement element, double[] localTotalDisplacements)
		{
			...
			{
				lastStresses[npoint] = materialsAtGaussPoints[npoint].UpdateConstitutiveMatrixAndEvaluateResponse(GLvec[npoint]);
			}

			return new Tuple<double[], double[]>(new double[123], lastStresses[materialsAtGaussPoints.Length - 1]);
		}
		...
		public virtual IMatrix StiffnessMatrix(IElement element)
		{
			...
			if (stiffnessCase == 1)
			{
				...
				for (int npoint = 0; npoint < materialsAtGaussPoints.Length; npoint++)
				{
					materialsAtGaussPoints[npoint].UpdateConstitutiveMatrixAndEvaluateResponse(GLvec[npoint]);
				}
