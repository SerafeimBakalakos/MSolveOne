...
namespace MGroup.FEM.Structural.Elements
{
	public class Rod2D : IStructuralFiniteElement
	{
		...
		{
			double[] globalStresses = CalculateStresses(element, localDisplacements).Item2; // item1 = strains
		...
		private double[] CalculateForces(IElement element, double[] localdDisplacements)
		{
		...
		public Tuple<double[], double[]> CalculateStresses(IElement element, double[] local_Displacements)
		{
			...
			double[] strains = null;
			double[] forces = CalculateForces(element, local_Displacements);
		...
		{
			return CalculateForces(element);
		}

		public double[] CalculateForces(IElement element) => CalculateForces(element, currentDisplacements);
