...
namespace MGroup.FEM.Structural.Elements
{
	public class Beam2DCorotational : Beam2DCorotationalAbstract
	{
		...
		public Beam2DCorotational(IList<Node> nodes, double youngModulus, double poissonRatio, double density, BeamSection2D beamSection)
		...
		public override void UpdateState(double[] nodeDisplacements)
		{
			displacementsOfCurrentIncrement.CopyFrom(nodeDisplacements);
			displacementsOfCurrentIncrement.SubtractIntoThis(lastDisplacements);
			currentDisplacements.CopyFrom(nodeDisplacements);
