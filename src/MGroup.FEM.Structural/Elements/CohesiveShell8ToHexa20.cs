	...
	public class CohesiveShell8ToHexa20 : IStructuralFiniteElement, IEmbeddedElement
	{
		...
		private double[] UpdateForces(IElement element, Matrix[] RtN3, double[] integrationCoeffs)
		{
			...
			for (int npoint1 = 0; npoint1 < nGaussPoints; npoint1++)
			{
				...
				for (int l = 0; l < 3; l++)
				{
					T_int_integration_coeffs[l] = lastTractions[npoint1][l] * integrationCoeffs[npoint1];
				}
		...
		public Tuple<double[], double[]> CalculateStresses(IElement element, double[] localTotalDisplacementsSuperElement)
		{
			double[][] Delta = new double[nGaussPoints][];
			...
			{
				lastTractions[i] = materialsAtGaussPoints[i].UpdateConstitutiveMatrixAndEvaluateResponse(Delta[i]);
			}
			return new Tuple<double[], double[]>(Delta[materialsAtGaussPoints.Length - 1], lastTractions[materialsAtGaussPoints.Length - 1]);
		}
		...
		{
			foreach (ICohesiveZoneMaterial3D m in materialsAtGaussPoints) m.CreateState();
		}
