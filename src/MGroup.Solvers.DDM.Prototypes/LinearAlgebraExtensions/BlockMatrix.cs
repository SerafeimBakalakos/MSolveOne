...
namespace MGroup.Solvers.DDM.Prototypes.LinearAlgebraExtensions
{
	public class BlockMatrix : IVectorMultipliable
	{
		...
		public BlockVector ExtractDiagonal()
		{
			Debug.Assert(numColBlocks == numRowBlocks);
			Debug.Assert(NumColumns == NumRows);
			var diagonal = new BlockVector(this.colsPerBlock);
			diagonal.Multiplicities = ColMultiplicities;
			for (int b = 0; b < this.numColBlocks; ++b)
			{
				diagonal.Blocks[b] = this.blocks[b, b].GetDiagonal();
			}
			return diagonal;
		}
