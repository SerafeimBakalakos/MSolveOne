...
namespace MGroup.FEM.Tests.IntegrationTests
{
	...
	public class IntegrationTests
	{
		...
		//	var solverFactory = new SkylineSolver.Builder();
		...
		public void SolveCantileverBeam2D()
		{
			...
			double expectedDisplacement = -2.08333333333333333e-5;
			INode monitorNode = model.NodesDictionary[2];
			IDofType monitorDof = StructuralDof.TranslationY;
			double computedDisplacement = algebraicModel.ExtractSingleValue(
				solver.LinearSystem.Solution, monitorNode, monitorDof);

			Assert.Equal(expectedDisplacement, computedDisplacement);
		}
		...
		private static void SolveLinearTrussExample()
		{
			...
			Assert.Equal(0.00053333333333333336, solver.LinearSystem.Solution.SingleVector[0], 10);
			Assert.Equal(0.0017294083664636196, solver.LinearSystem.Solution.SingleVector[1], 10);
		}
