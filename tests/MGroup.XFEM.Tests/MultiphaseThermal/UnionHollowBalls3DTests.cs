...
namespace MGroup.XFEM.Tests.MultiphaseThermal
{
	public static class UnionHollowBalls3DTests
	{
		private static readonly string outputDirectory = Path.Combine(
			Directory.GetParent(Directory.GetCurrentDirectory()).Parent.FullName, "Resources", "union_hollow_balls_3D_temp");
		private static readonly string expectedDirectory = Path.Combine(
			Directory.GetParent(Directory.GetCurrentDirectory()).Parent.FullName, "Resources", "union_hollow_balls_3D");
		...
		private static PhaseGeometryModel CreatePhases(XModel<IXMultiphaseElement> model)
		{
			...
			for (int p = 0; p < ballsInternal.Length; ++p)
			{
				...
				var externalCurve = new SimpleLsm3D(externalPhase.ID, model.Nodes.Values.OrderBy(n => n.ID).ToList(), 
					ballsExternal[p]);
				...
				var internalLsm = new SimpleLsm3D(2 * p + 2, model.Nodes.Values.OrderBy(n => n.ID).ToList(), ballsInternal[p]);
