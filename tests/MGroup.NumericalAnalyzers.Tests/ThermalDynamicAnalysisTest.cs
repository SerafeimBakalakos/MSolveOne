namespace MGroup.Analyzers.Tests
{
	...
	public class ThermalDynamicAnalysisTest
	{
		...
		private static IVectorView SolveModel(Model model)
		{
			...
			var provider = new ProblemThermal(model, algebraicModel, solver);

			var childAnalyzer = new LinearAnalyzer(model, algebraicModel, solver, provider);
			var parentAnalyzer = new ThermalDynamicAnalyzer(model, algebraicModel, solver, provider, childAnalyzer, 0.5, 0.5, 1000);
