namespace MGroup.Analyzers.Tests
{
	...
	public static class NewmarkDynamicAnalysisTests
	{
		...
		private static void TestBatheImplicitAnalysisExample()
		{
			...
			var parentAnalyzerBuilder = new NewmarkDynamicAnalyzer.Builder
				(model, algebraicModel, solver, provider, childAnalyzer, 0.28, 3.36);
			...
			watchDofs.Add((n, StructuralDof.TranslationX));
			watchDofs.Add((n, StructuralDof.TranslationY));
			childAnalyzer.LogFactory = new LinearAnalyzerLogFactory(watchDofs, algebraicModel);
			//childAnalyzer.LogFactories[subdomainID] = new LinearAnalyzerLogFactory(new int[] { 0, 1 }, solver.LinearSystem);
			...
			Assert.Equal(2.2840249264795207, log.DOFValues[watchDofs[0].node, watchDofs[0].dof], 8);
			Assert.Equal(2.4351921891904156, log.DOFValues[watchDofs[1].node, watchDofs[1].dof], 8);
