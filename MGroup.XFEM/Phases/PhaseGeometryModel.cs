	...
	public class PhaseGeometryModel : IGeometryModel
	{
		...
		public void InteractWithMesh()
		{
			...
			foreach (IPhase phase in Phases.Values)
			{
				...
				{
					phase.InteractWithElements(physicalModel.Elements.Values);
				}
			}
			if (defaultPhase != null)
			{
				defaultPhase.InteractWithElements(physicalModel.Elements.Values);
			}

			foreach (IPhaseMeshInteractionObserver observer in InteractionObservers)
			{
				observer.Update();
			}
		}
		...
		private void InteractWithNodes()
		{
			if (calcPhasesNodesInteractions)
			{
				...
				foreach (IPhase phase in Phases.Values)
				{
					if (phase != defaultPhase)
					{
						phase.InteractWithNodes(physicalModel.Nodes.Values);
					}
				}
				if (defaultPhase != null)
				{
					defaultPhase.InteractWithNodes(physicalModel.Nodes.Values);
				}
			}
